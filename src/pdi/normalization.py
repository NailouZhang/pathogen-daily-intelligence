...
def make_scholarly_work(records: list[dict[str, Any]], work_id: str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "title":{"original":title_rec.get("title") or "","translated_zh":next((r.get("translated_title_zh") for r in records if r.get("translated_title_zh")),None),"language":title_rec.get("language")},
        "abstract":{"original":abstract_rec.get("abstract"),"translated_zh":next((r.get("translated_abstract_zh") for r in records if r.get("translated_abstract_zh")),None),"source":abstract_rec.get("source_id"),"sentences":abstract_rec.get("abstract_sentences") or []},
        "display_summary":{"zh":next((r.get("display_summary_zh") for r in records if r.get("display_summary_zh")),None),"en":next((r.get("display_summary_en") for r in records if r.get("display_summary_en")),None)},
        ...
        "source_records":[{"source_id":r.get("source_id"),"source_record_id":r.get("source_record_id"),"url":r.get("url"),"query_group":r.get("query_group"),"retrieved_at":r.get("retrieved_at"),"full_text_available":r.get("full_text_available"),"open_access":r.get("open_access")} for r in records],
        "conflicts":conflicts,"ai_analysis":next((r.get("demo_ai_analysis") for r in records if r.get("demo_ai_analysis")),None),"full_text":{"available":False,"source":None,"url":None,"sections":[]},"processing_audit":{},"quality":{"has_abstract":bool(abstract_rec.get("abstract")),"source_count":len(records),"open_full_text_available":any(bool(r.get("full_text_available")) for r in records)},"first_seen_at":first_seen,"last_seen_at":utc_now_iso(),"last_updated_at":utc_now_iso()
    }
...
def normalize_news_article(rec: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"schema_version":"1.0","article_id":article_id,"canonical_url":canonicalize_url(rec.get("canonical_url") or rec.get("url")),"original_url":rec.get("url"),"source":{"source_id":rec.get("source_id"),"name":rec.get("source_name"),"domain":rec.get("domain"),"organization_type":rec.get("source_category"),"reliability_tier":rec.get("source_tier","unknown")},"title":{"original":rec.get("title") or "","translated_zh":rec.get("translated_title_zh"),"language":rec.get("language")},"published_at":rec.get("published_at"),"first_seen_at":rec.get("retrieved_at"),"last_seen_at":rec.get("retrieved_at"),"content":{"excerpt":rec.get("excerpt"),"original_excerpt":rec.get("excerpt"),"analysis_text":rec.get("excerpt"),"translation_text":rec.get("excerpt"),"translated_excerpt_zh":rec.get("translated_excerpt_zh"),"sentences":rec.get("content_sentences") or [],"extraction":None},"display_summary":{"zh":rec.get("display_summary_zh"),"en":rec.get("display_summary_en")},"translation_audit":None,"entities":{"pathogens":[],"diseases":[],"hosts":[],"event_type":None,"country":None,"admin1":None,"city":None,"event_date":None,"confirmed_cases":None,"probable_cases":None,"suspected_cases":None,"deaths":None},"official_references":[],"fingerprints":{"title":stable_hash(normalize_title(rec.get("title"))),"url":stable_hash(canonicalize_url(rec.get("canonical_url") or rec.get("url")))},"classification":{},"ai_analysis":rec.get("demo_ai_analysis"),"event_id":None,"retrieval_audit":{"query_group":rec.get("query_group"),"query":rec.get("query"),"original_source":rec.get("original_source"),"content_fetch":None},"processing_audit":{}}
