...
def _source_record(
    ...
    *,
    ...
    return {
        "record_type": "news_source",
        "source_id": source["source_id"],
        "source_name": source.get("name"),
        "source_tier": source.get("reliability_tier", "unknown"),
        "source_category": source.get("category", "discoverer"),
        "source_record_id": stable_hash(canon or title),
        "query_group": task.group_id,
        "query": task.query,
        "language": language or task.language,
        "title": normalize_space(title),
        "excerpt": normalize_space(excerpt) or None,
        "content_sentences": sentence_split(excerpt, "N"),
...
def _publisher_link_from_google_entry(entry: Any) -> str | None:
    """Prefer the publisher URL embedded in Google News RSS HTML when present."""
    html_candidates = [entry.get("summary"), entry.get("description"), entry.get("content", [{}])[0].get("value") if entry.get("content") else None]
    for raw in html_candidates:
        ...
        for anchor in soup.find_all("a", href=True):
            ...
            host = urlsplit(href).netloc.casefold()
            if href.startswith("http") and "news.google." not in host and "google.com" not in host:
                return href
    return None


def _clean_google_summary(raw: Any) -> str | None:
    if not raw:
        return None
    soup = BeautifulSoup(str(raw), "lxml")
    text = normalize_space(soup.get_text(" ", strip=True))
    return text[:1800] or None
...
def collect_google_news(client: HttpClient, source: dict[str, Any], tasks: list[QueryTask], window: CoverageWindow) -> SourceResult:
    ...
    for task in tasks:
        ...
        days = max(1, (datetime.fromisoformat(window.end) - datetime.fromisoformat(window.start)).days + 1)
        q = f"({task.query}) when:{days}d"
        url = (
            "https://news.google.com/rss/search?q="
            + quote_plus(q)
            + f"&hl={quote_plus(loc.get('hl', 'en-US'))}&gl={quote_plus(loc.get('gl', 'US'))}&ceid={quote_plus(loc.get('ceid', 'US:en'))}"
        )
        response, audit = client.request("GET", url)
        result.audits.append(_audit_dict(audit))
        if response is None:
            result.errors.append(audit.error or "Google News RSS failed")
            continue
        ...
        for entry in feed.entries[: task.limit]:
            ...
            publisher_url = _publisher_link_from_google_entry(entry)
            link = publisher_url or entry.get("link", "") or ""
            result.records.append(
                _source_record(
                    ...
                    excerpt=_clean_google_summary(entry.get("summary")),
...
def collect_gdelt(client: HttpClient, source: dict[str, Any], tasks: list[QueryTask], window: CoverageWindow) -> SourceResult:
    result = SourceResult(source["source_id"], "success")
    start = window.start.replace("-", "") + "000000"
    end = window.end.replace("-", "") + "235959"
    for task in tasks:
        result.query_count += 1
        ...
        result.audits.append(_audit_dict(audit))
        if not data:
            result.errors.append(audit.error or "GDELT failed")
            continue
        for rec in data.get("articles", []):
            result.records.append(
                _source_record(
                    ...
                    title=rec.get("title", ""),
                    url=rec.get("url", "") or "",
                    published=rec.get("seendate"),
                    excerpt=rec.get("snippet") or rec.get("description"),
                    language=rec.get("language") or task.language,
                    original_source=rec.get("domain"),
                )
            )
    if result.errors and not result.records:
        result.status = "failed"
...
def collect_reliefweb(client: HttpClient, source: dict[str, Any], tasks: list[QueryTask], window: CoverageWindow) -> SourceResult:
    ...
    for task in tasks:
        ...
        payload = {
            ...
            "filter": {"field": "date.created", "value": {"from": window.start + "T00:00:00+00:00", "to": window.end + "T23:59:59+00:00"}},
            "limit": min(task.limit, 30),
            "sort": ["date.created:desc"],
            "fields": {"include": ["id", "url", "title", "date.created", "source.name", "body", "country.name", "language.name"]},
        }
        data, audit = client.post_json(source["base_url"], json=payload)
        result.audits.append(_audit_dict(audit))
        if not data:
            result.errors.append(audit.error or "ReliefWeb failed")
            continue
        for item in data.get("data", []):
            ...
            result.records.append(
                _source_record(
                    ...
                    excerpt=(fields.get("body") or "")[:3000],
                    language=((fields.get("language") or [{}])[0].get("name") if fields.get("language") else task.language),
                    original_source=", ".join(x for x in src_names if x),
                )
            )
    if result.errors and not result.records:
        result.status = "failed"
...
def collect_official_page(client: HttpClient, source: dict[str, Any], tasks: list[QueryTask], window: CoverageWindow) -> SourceResult:
    ...
    if response is None:
        result.status = "failed"
        result.errors.append(audit.error or "Official page failed")
        return result
    ...
    for link in soup.find_all("a", href=True):
        ...
        result.audits.append(_audit_dict(detail_audit))
        result.query_count += 1
        published = None
        excerpt = None
        detail_title = title
...
def collect_news(client: HttpClient, source: dict[str, Any], tasks: list[QueryTask], window: CoverageWindow) -> SourceResult:
    ...
    if adapter == "google_news":
        return collect_google_news(client, source, tasks, window)
    if adapter == "gdelt":
        return collect_gdelt(client, source, tasks, window)
    if adapter == "reliefweb":
        return collect_reliefweb(client, source, tasks, window)
    if adapter == "official_page":
        return collect_official_page(client, source, tasks, window)
    if adapter == "registry_only":
        return SourceResult(source.get("source_id", "unknown"), "disabled")
