# Pathogen Daily Intelligence v1.7.0

GitHub Actions + GitHub Pages + Streamlit 的中英文病原每日情报系统。

## v1.7.0 修复摘要（基于 2026-07-15 实际输出的人工审计）

- **网页布局**：修复“今日要闻”与“疫情与公共卫生”/“学术文献”板块 100% 重复
  展示同一批卡片的问题。
- **模型解读 × 翻译**：深度解读阶段不再先于翻译执行、也不再覆盖翻译审计，
  修复了审计栏“先 passed 又 failed”的自相矛盾记录，并让翻译不再被解读阶段
  的失败连带拖垮。
- **翻译校验**：修复数字千分位分组（如印度式 “2,00,000”）被误判为
  “数字被改动”从而导致翻译在全部 provider 上失败的问题。
- **事件聚合去重**：放宽“地点未知”和“事件类型家族不一致”两处过严的相似度
  惩罚，减少同一真实事件被拆成多张事件卡片的情况。
- **内容抓取**：已知 JS 专属渲染的聚合站点（当前为 msn.com）不再浪费三次
  跳转重试预算，第一时间走 RSS 摘要兜底并标注真实限制原因。

详见 `docs/v1.7审计与修复报告.md`（问题现象、根因、修复方式、已知局限、
建议的回归测试命令）。
...
- 远程模型顺序失败后，使用 MarianMT 英中模型进行 Python 本地翻译。
- 远程模型译文通过格式校验后，再以本地标题译文作为独立质量信号。
- 本地参考译文不是绝对标准；术语、数字、否定、不确定性和中文自然度由确定性规则共同判定。
- Google News 固定模板会在翻译前移除；正文为空时只翻译标题，不生成伪摘要。
- 翻译、正文分析和每日综合使用独立审计状态。
- DOI/PDF 身份校验不再允许 DOI 字符串单独覆盖题名与作者冲突。
